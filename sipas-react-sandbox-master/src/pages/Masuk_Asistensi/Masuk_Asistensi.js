...
import MasukAsistensiView from './Masuk_AsistensiView';

const dataMasukAsistensi = [
    {
        id: 1, 
...
const routes = [].concat(
    [{exact: true}],
    [...dataMasukAsistensi].map(({route, exact, component}) => ({
        exact,
        path: route,
...
const Masuk_Asistensi = ({...props}) => {
    return(
        <MasukAsistensiView 
        routes={routes}
        dataMasukAsistensi={dataMasukAsistensi}
        {...props}
        />
    );
};

export default memo(Masuk_Asistensi);